...
document.addEventListener("DOMContentLoaded", () => {
  const lkSwiperCourse = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](".lk-swiper", {
    ...
    pagination: {
      el: '.lk-swiper__course-pagination',
      type: 'bullets'
    }
  });
  if (document.querySelectorAll('.textarea-message')) {
    const messageTextarea = document.querySelectorAll('.textarea-message');
    messageTextarea.forEach(el => {
  ...
  if (document.querySelector(".lk-accordion-item")) {
    document.querySelectorAll(".lk-accordion-item").forEach(item => {
      item.querySelector(".lk-accordion-item-header").addEventListener("click", () => {
        document.querySelectorAll(".lk-accordion-item").forEach(item => {
  ...
  document.querySelectorAll('.open-modal').forEach(function (button) {
    button.addEventListener('click', function () {
      ...
      $.fancybox.open({
        ...
        touch: false,
        // Отключает жесты для мобильных устройств
        beforeShow: function (instance, slide) {
          // Удаление возможности перетаскивания с помощью CSS
          $('.fancybox-content').css('pointer-events', 'none');
  ...
  document.querySelectorAll('.lk-close-modal').forEach(function (button) {
    button.addEventListener('click', function () {
      $.fancybox.close();
    });
  });
  ...
  inputs.forEach((input, index) => {
    ...
    input.addEventListener('keydown', function (ev) {
      if (!input.value && ev.key === 'Backspace') {
